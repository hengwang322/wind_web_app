...
from src.data import FARM_LIST, FARM_NAME_LIST
...
def format_title(string, acronym_list=['UV'], splitter='_'):
    """Convert strings with non-space splitters to title format.
    Check words in acronym_list and convert all chars to capital letters.
    """
    format_words = []
    for word in string.split(splitter):
        if word.lower() in [acronym.lower() for acronym in acronym_list]:
            word = word.upper()
        else:
            word = word.title()
